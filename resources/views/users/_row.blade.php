<tr>
    ...
    <td>
        ...
        <span class="note">Último login:
            {{ $user->lastLogin ? $user->lastLogin->created_at->format('d/m/Y h:ia') : 'N/A' }}</span>
    </td>
    <td class="text-right">
        @if ($user->trashed())
        ...
        @else
            <form action="{{ route('user.trash', $user) }}" method="POST">
                ...
                <a href="{{ route('user.edit', $user) }}" class="btn btn-outline-secondary btn-sm"><span
